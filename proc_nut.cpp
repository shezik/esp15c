						   ...
						   rom_word_t data)
{
	...
	if (! nut_reg->rom [page][bank])
    {
		Serial.printf("write to nonexistent ROM location %04x (bank %d)\n", addr, bank);
		return;
    }
	if (! nut_reg->rom_writeable [page][bank])
    {
		Serial.printf("write to non-writeable ROM location %04x (bank %d)\n", addr, bank);
		return;
    }
...
{	
	...
	if ((*pt (nut_reg)) == (WSIZE - 1))
    {
		nut_reg->g [1] = 0;
#ifdef WARNING_G
		Serial.printf("warning: c to g transfer with pt=13\n");
#endif
    }
	else
...
{	
	...
	if ((*pt (nut_reg)) == (WSIZE - 1))
    {
		;
#ifdef WARNING_G
		Serial.printf("warning: g to c transfer with pt=13\n");
#endif
    }
	else
    {
...
static void op_c_exch_g (nut_reg_t *nut_reg, int opcode)
{
	...
	if ((*pt (nut_reg)) == (WSIZE - 1))
    {
		nut_reg->g [1] = 0;
#ifdef WARNING_G
		Serial.printf("warning: c exchange g with pt=13\n");
#endif
    }
	else
    {
								   ...
								   rom_word_t *opcode)
{
	...
	if (buf [0] == '#')  /* comment? */
		...
		if (buf [4] != ':')
		{
			Serial.printf("invalid object file format\n");
			return (false);
		}
		
		if (! parse_hex (& buf [0], 4, & a))
		{
			Serial.printf("invalid address %o\n", a);
			return (false);
		}
		
		if (! parse_hex (& buf [5], 3, & o))
		{
			Serial.printf("invalid opcode %o\n", o);
			return (false);
		}
...
bool nut_read_object_file (nut_reg_t *nut_reg, const char *filename)
{
	...
	if (! f)
    {
		fatal(4, "Error opening object file: %s\n", filename);
		return false;
    }
